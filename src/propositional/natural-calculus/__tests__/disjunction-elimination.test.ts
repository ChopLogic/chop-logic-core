...
describe("disjunctionElimination", () => {
	it("should infer C from (A ∨ B), (A => C), and (B => C)", () => {
		...
		const implication1: PropFormula = {
			operator: Operator.Implies,
			values: [A, C],
		};
		const implication2: PropFormula = {
			operator: Operator.Implies,
			values: [B, C],
		};

		expect(
			disjunctionElimination([implication1, disjunction, implication2]),
		).toEqual([C]);
	});
	...
	it("should throw an error if the rule is not applicable", () => {
		...
		expect(() => disjunctionElimination([disjunction])).toThrow(
