...
describe('conjunctionElimination', () => {
  it('should correctly eliminate conjunctions into their conjuncts', () => {
    ...
    const conjunctionAB: PropFormula = { operator: Operator.And, values: [A, B] };
    const conjunctionCD: PropFormula = { operator: Operator.And, values: [C, D] };
  ...
  it('should throw an error if the input formulas are not all conjunctions', () => {
    ...
    expect(() => conjunctionElimination([A, B])).toThrow('Conjunction Elimination is not applicable for the given formulas.');
  });
});
