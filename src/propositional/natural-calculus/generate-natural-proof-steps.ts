import { PropFormula, PropProofStep } from '../../models';
 ...
 */
export function generateNaturalProofSteps<T>(input: NaturalProofStepInput<T>): PropProofStep[] {
  const { index, level, step, assumptionIndex } = input;

  if (step === Step.Derivation) {
    return buildDerivedSteps({ index, level, assumptionIndex, payload: input.payload as DerivedPayload });
  }

  return [buildBaseStep({ index, level, step, assumptionIndex, payload: input.payload as BasePayload })];
}
