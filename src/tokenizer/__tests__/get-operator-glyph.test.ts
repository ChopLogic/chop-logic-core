...
describe("getOperatorGlyph", () => {
	it("should return the correct glyph for logical operators", () => {
		...
		expect(getOperatorGlyph(Operator.ReversedImplies)).toBe(
			Glyph.ReversedImplication,
		);
		...
		expect(getOperatorGlyph(Operator.ReversedAntiImplies)).toBe(
			Glyph.ReversedAntiImplication,
		);
	...
	it("should throw an error for an unrecognized operator", () => {
		expect(() => getOperatorGlyph(Operator.Var)).toThrow(
			'Cannot convert operator "VAR" to a glyph.',
		);
	});
});
